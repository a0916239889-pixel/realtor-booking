/**
 * 房產顧問 bot(@mrbin)推「律廷官方 Line AI 客服機器人」admin 群 — HITL 接手通知用
 *
 * 2026-06-04: HITL 通知必須走房產顧問 bot 推律廷監控群(台灣房屋三重國小捷運店 bot 不在那群,所以不能用 notifyLineGroup)。
...
/**
 * 房產顧問 bot 推 admin 監控群，回傳可供 outbox 判斷是否值得重試的完整原因。
 * 台灣房屋三重國小捷運店 bot 不在這個群組，admin 通知不得改走 LINE_CHANNEL_* token。
 */

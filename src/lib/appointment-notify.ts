 ...
 * 新預約成立 → ① email 通知系統擁有者 ② LINE 通知系統擁有者（律廷 admin 群）③ email 確認給客戶。
 *
 * 🔴 寄信安全閘（對齊寄信紅線）：
...
const BINGE_EMAIL = process.env.APPOINTMENT_ADMIN_EMAIL || OWNER.email;
const APPOINTMENT_BASE_URL = process.env.APPOINTMENT_BASE_URL || "http://localhost:3000";
...
export function buildAppointmentIcs(opts: {
  ...
  return [
    ...
    `SUMMARY:${icsEsc(`與律廷預約：${opts.name}`)}`,
...
/** 房產顧問 CIS 亮色 email 外殼（天藍 header + 橘 CTA + 深字,RWD 手機不跑版,內聯 style）*/
...
<table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:#FFFFFF;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(28,45,58,0.08)">
  <tr><td style="background:linear-gradient(135deg,#4EC4DC,#2BA9C4);padding:22px 32px">
    <div style="font-size:13px;color:#E8F7FB;letter-spacing:1px">${esc(OWNER.name)} ${esc(OWNER.title)}</div>
  ...
  <tr><td style="background:#F7FAFB;padding:20px 32px;border-top:1px solid #E3EAEE">
    <div style="font-size:13px;color:#7A8896;line-height:1.8">
      ${esc(OWNER.name)} ‧ ${esc(OWNER.title)}<br>
...
<table role="presentation" class="container" width="560" cellpadding="0" cellspacing="0" style="width:560px;max-width:560px;background:#FFFFFF;border:1px solid #E3EAEE;border-radius:12px;overflow:hidden">
  ...
  <tr><td class="px" style="padding:22px 30px 4px">
    <div style="font-size:13px;color:#7A8896">${esc(OWNER.name)} · ${esc(OWNER.title)}</div>
  ...
  <tr><td style="padding:16px 30px;border-top:1px solid #EDF2F4">
    <div style="font-size:13px;color:#7A8896;line-height:1.7">${esc(OWNER.name)} · ${esc(OWNER.company)}　LINE / 電話 ${esc(OWNER.phone)}</div>
  </td></tr>
</table>
</td></tr></table></body></html>`;
}
...
export async function sendCustomerAppointmentConfirmation(a: NotifyInput): Promise<boolean> {
  ...
  const custBody = `
    <p style="font-size:16px;line-height:1.7;margin:0 0 16px">${esc(a.name)} ${honor}你好,律廷已經收到你的預約,時段幫你安排好了。</p>
    ...
    <p style="text-align:center;font-size:13px;color:#7A8896;line-height:2;margin:6px 0 0">
      <a href="${esc(manageUrl)}" style="color:#1F6F83;text-decoration:none">改期 / 取消</a>${mapUrl ? ` · <a href="${esc(mapUrl)}" style="color:#1F6F83;text-decoration:none">地圖導航</a>` : ""} · <a href="${esc(ABIN_PRIVATE_LINE_URL)}" style="color:#1F6F83;text-decoration:none">加律廷 LINE</a>
    </p>
    <p style="font-size:14px;color:#7A8896;line-height:1.7;margin:18px 0 0">有任何狀況,直接回覆這封信或加律廷 LINE 都可以。</p>
  `;
  return sendAppointmentEmailAndRecord({
    ...
    html: transactionalEmailLayout({
      title: "預約已確認",
      preheader: `${esc(a.name)} ${honor}你好,律廷已收到你的預約 · ${slotTw}`,
      bodyHtml: custBody,
    }),
  });
}
...
export async function sendCustomerAppointmentConfirmationRequest(a: NotifyInput): Promise<boolean> {
  ...
  return sendAppointmentEmailAndRecord({
    ...
    purpose: "customer_confirmation_request",
    to: a.email,
    subject: `請在 15 分鐘內確認預約 · ${OWNER.alias}（${OWNER.name}）`,
    html: transactionalEmailLayout({
      title: "時段已暫時保留",
...
export async function sendCustomerAppointmentChangeEmail(
  ...
  const body = `
    ...
    <p style="font-size:14px;color:#7A8896;line-height:1.7;margin:18px 0 0">
      ${isCancel ? "如果還需要重新預約,可以回信或直接加律廷 LINE。" : "確認信與行事曆請以這封更新後的資訊為準。"}
    </p>
  `;
  return sendAppointmentEmailAndRecord({
    appointmentId: a.id,
    ...
    to: a.email,
    subject: isCancel ? `預約已取消 · ${OWNER.alias}（${OWNER.name}）` : `預約已改期 · ${OWNER.alias}（${OWNER.name}）`,
    html: transactionalEmailLayout({
      title,
...
export async function sendCustomerAppointmentReminder(a: NotifyInput): Promise<boolean> {
  ...
  const slotTw = formatSlotRangeTw(a.slotAt, a.slotEndAt);
  const body = `
    <p style="font-size:16px;line-height:1.7;margin:0 0 16px">${esc(a.name)} ${GENDER_HONOR[a.gender] || ""}你好,提醒你明天有和律廷的預約。</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:15px;line-height:1.9;background:#F7FAFB;border-radius:10px">
      <tr><td style="padding:14px 16px">
        <div>📅 時段　<b>${esc(slotTw)}</b></div>
        <div style="margin-top:4px">📍 方式　<b>${esc(meetTypeLabel(a.meetType))}</b></div>
    ...
    <p style="font-size:14px;color:#7A8896;line-height:1.7;margin:18px 0 0">若需要改期或取消,請直接回信或加律廷 LINE。</p>
  `;
  return sendAppointmentEmailAndRecord({
    appointmentId: a.id,
    purpose: "customer_reminder",
...
export async function notifyNewAppointment(
  ...
  const locEmailHtml = loc
    ...
    : "";
  // 給 LINE 用的地點文字。固定地點（公司面談/分公司/台灣房屋三重國小捷運店工作室）律廷本來就知道在哪，

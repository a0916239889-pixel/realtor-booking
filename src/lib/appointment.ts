...
export async function createAppointment(data: {
  ...
  /** 律廷核准自訂地點後產生的一次性 token；只有 meetType=custom 才會核銷。 */
...
/** 未來已佔時段（給前台算可約空檔扣除）
 *
 *  🔴 2026-09-09 修正重複預約：原本這兩處寫 `appointment_id NOT LIKE '__%'`，
 *     想排除的是 `__grid15_migrated__`、`__meeting_buffers_v2__` 這兩列標記。
 *     但 SQL 的 LIKE 裡 `_` 是「任一個字元」的萬用字元，`'__%'` 等於「任何兩個字以上」——
 *     **所有真實預約都被濾掉了**，這個函式永遠回空陣列，前台等於沒扣掉已佔時段。
 *     沒接 Google 日曆時（日曆 busy 會蓋掉這個 bug）就會把同一格賣兩次。
 *     改用 LEFT(...)='__' 比對字面上的兩個底線。 */

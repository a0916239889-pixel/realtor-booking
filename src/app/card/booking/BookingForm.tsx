...
function qualificationFields(mode: BookingMode, intent: string) {
  ...
  if (intent === "buy" || intent === "rent") {
    return [
      { key: "area", label: "希望區域", placeholder: "例：三重、蘆洲、中山、大同", required: true },
  ...
  if (intent === "sell") {
    return [
      { key: "propertyAddress", label: "物件大約位置", placeholder: "例：三重區正義北路，填到路名即可", required: true },
...
export default function BookingForm() {
  ...
  const validate = (): FieldErrors => {
    ...
    if (meetType === "custom" && !customReady) next.meetType = "指定地點必須使用律廷核准後提供的專屬連結。";
    ...
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email.trim())) next.email = "請填寫正確 Email，預約確認信與改期連結會寄到這裡。";
      ...
      <main className={styles.page}>
        <div className={styles.shell}>
          <header className={styles.header}>
            <div className={styles.brand}>周律廷 房產顧問</div>
            ...
            <p className={styles.statusText}>
              {expired
                ...
                : pending
                  ...
                  : "預約已成立。請先把時間加入行事曆，並收下下面的「管理預約」連結 —— 要改期或取消都用它。"}
            </p>
            {/* 2026-09-09：目前寄件網域還沒驗證，客戶信箱收不到確認信，
                所以管理連結一定要在畫面上講清楚，不能叫人去信箱找。 */}
            {!pending ? (
              <div className={styles.warningNotice}>
                這個畫面的「管理預約」連結請先存起來（截圖或加書籤）。要改時間或取消，用它最快；
                找不到也沒關係，直接用下面的 LINE 或電話找律廷就行。
              </div>
            ) : null}
            {pending && !expired ? (
  ...
  return (
    <main className={styles.page}>
      <div className={styles.shell}>
        <header className={styles.header}>
          <div className={styles.brand}>周律廷 房產顧問</div>
          <h1 className={styles.title}>預約與律廷聊聊</h1>
        ...
        <form className={styles.form} onSubmit={submit} noValidate>
          ...
          {bookingMode && intentKey ? (
            <section className={styles.section}>
              ...
              {approvalState === "approved" && !approvedLocation ? (
                <div className={styles.errorNotice}>核准連結缺少已同意的地點，無法開放指定地點。請聯絡律廷重新產生連結。</div>
              ) : null}
              {approvalState === "invalid" ? (
                <div className={styles.errorNotice}>指定地點核准連結已失效、已使用或被撤銷，請重新聯絡律廷。</div>
              ) : null}
              ...
              {!customReady && approvalState !== "checking" ? (
                <div className={styles.inlineNotice}>
                  需要指定其他地點？請先
                  {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">加律廷私人 LINE</a>
                  {" "}確認。取得專屬連結後，系統會直接帶入核准地點。
                </div>
              ) : null}
              {meetType === "custom" && approvedLocation ? (
                <div className={styles.field}>
                  <span className={styles.label}>律廷已核准的指定地點</span>
                  ...
                  <p className={styles.sectionHint}>此地點已鎖定，若要更換請先與律廷重新確認。</p>
                </div>
              ) : null}
            </section>
          ) : null}
              ...
              <div ref={registerRef("slot")} tabIndex={-1}>
                ...
                {!loadingSlots && !slotError && days.length === 0 ? (
                  <div className={styles.inlineNotice}>
                    ...
                    {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">直接聯絡律廷</a>。
                  </div>
                ) : null}
                {days.length ? (
                  <>
          ...
          {selectedStart && duration && bookingMode && selectedIntent ? (
            <section className={styles.section}>
              ...
              <Turnstile onToken={setTurnstileToken} onError={() => setTurnstileBroken(true)} />
              {turnstileBroken ? (
                <div className={styles.errorNotice} role="alert">
                  防機器人驗證載入失敗，請重新整理頁面；若還是不行，
                  {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">直接用 LINE 跟我約</a>。
                </div>
              ) : null}
              ...
              {/* 預設是「送出即成立」（後端 requireConfirmation 預設 false）。
                  若日後設了 APPOINTMENT_REQUIRE_CONFIRMATION=1，這段文案要改回「先保留
                  {BOOKING_CONFIRMATION_HOLD_MINUTES} 分鐘、要去 Email 點確認」。 */}
              <p className={styles.submitHint}>
                送出後預約就成立，時段會直接鎖起來。下一頁會給你一個管理連結，改期或取消都用它。
              </p>
            </section>
          ) : null}
        </form>

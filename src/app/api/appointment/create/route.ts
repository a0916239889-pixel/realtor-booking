 ...
 * 排完任務後立刻在背景跑一輪佇列。
 *
 * 🔴 2026-09-09 加的。原本只有 enqueue，而整包程式沒有任何地方會去執行那些任務，
 *    等於通知信永遠不會寄出去。跑在背景 + 自己吞例外，寄信失敗不會害預約失敗。
 */
async function enqueueAndRunCreatedAppointmentTasks(appt: AppointmentRow): Promise<void> {
  await enqueueCreatedAppointmentTasks(appt);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (existing) {
      await enqueueAndRunCreatedAppointmentTasks(existing);
    ...
    if (error instanceof AppointmentLocationApprovalError) {
      const messages: Record<string, string> = {
        approved_location_missing: "指定地點核准資料不完整，請重新向律廷確認。",
      ...
      return NextResponse.json(
        {
          error: messages[error.message] || "指定地點尚未取得律廷核准，或核准連結已失效。",
          code: error.message,
        },
        { status: 403 },
      );
    ...
    if (error instanceof GoogleCalendarUnavailableError) {
      return NextResponse.json(
        {
          error: "目前無法確認律廷的行事曆，為避免重複預約，這次沒有建立預約。請稍後再試。",
